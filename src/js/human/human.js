...
function buildSkeletonFromLegacyBones(mesh, legacyGeometry) {
    const boneData = legacyGeometry && legacyGeometry.bones
    ...
    const bones = boneData.map((data, index) => {
        ...
        bone.name = data && data.name ? data.name : `bone_${index}`
    ...
    for (let i = 0; i < boneData.length; i++) {
        const parentIndex = boneData[i] && Number.isInteger(boneData[i].parent) ? boneData[i].parent : -1
    ...
    loadModel() {
        ...
        return new Promise((resolve, reject) => {
            ...
            .then((json) => {
                ...
                const texturePath = self.texturePath && (typeof self.texturePath === "string") ? self.texturePath : extractUrlBase(modelUrl)
